--[[----------------------------------------------------------------------------
  ...
  Script        : It-Alien_silence_detector.lua
...
manifest.description = "erase/remove silence from a sample by user definition."
manifest.actions = {}

manifest.actions[#manifest.actions + 1] = {
  name = "SampleEditor:Process:Silence Detector",
...
function is_under_threshold(int_frame)
...
function show_dialog() 
	...
	valigner:add_child(row_db);
	valigner:add_child(column_slider);
	valigner:add_child(row_time);
	valigner:add_child(column_mode);
