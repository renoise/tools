...
vBitmap.BIT_COUNT = {1,4,8,16,24,32}

--[[
vBitmap.BMP_COMPRESSION = {
  UNCOMPRESSED = 0,
...
vBitmap.HEADER_SIZE = 54
vBitmap.PIXELS_PER_METER = 2834
...
function vBitmap:__init(...)
  ...
  -- (vBitmap.BIT_COUNT) NB: only 24-bit is tested!!
...
function vBitmap.split_bits(num,endian)
...
function vBitmap:get_bytes_padding()
  --TRACE("vBitmap:get_bytes_padding()",self)
  return (self.width)%4
end

-------------------------------------------------------------------------------

function vBitmap:get_bitmap_size_in_bytes()
  --TRACE("vBitmap:get_bitmap_size_in_bytes()",self)
...
function vBitmap:get_total_size_in_bytes()
  --TRACE("vBitmap:get_total_size_in_bytes()",self)
  return vBitmap.HEADER_SIZE + self:get_bitmap_size_in_bytes()
end

-------------------------------------------------------------------------------
--- this method will create the bitmap table

function vBitmap:create()

  self.bitmap = {}

  local size_bytes = vBitmap.split_bits(self:get_total_size_in_bytes(),true)
  local bitmap_bytes = vBitmap.split_bits(self:get_bitmap_size_in_bytes(),true)
  local width_bytes = vBitmap.split_bits(self.width,true)
  local height_bytes = vBitmap.split_bits(self.height,true)
  local ppm_bytes = vBitmap.split_bits(vBitmap.PIXELS_PER_METER,true)
  ...
  self.bitmap[11] = vBitmap.HEADER_SIZE
  self.bitmap[12] = 0
  self.bitmap[13] = 0
  self.bitmap[14] = 0
