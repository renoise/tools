...
for k = 1,col_count do
  ...
  if has_changed then
    if mute_state then
      ...
      local pos = xbuffer:get_scheduled_pos(xStream.SCHEDULE.LINE)
      xbuffer:schedule_note_column(xnotecol,k,pos.lines_travelled)
      xbuffer:schedule_note_column(xnotecol,k,pos.lines_travelled+1)
    end 
  else
    if mute_state then
