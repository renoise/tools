...
function xNoteColumn:__init(args)
  ...
  for token,value in pairs(args) do
    if (table.find(xNoteColumn.tokens,token)) then
      --print("*** xNoteColumn:__init - token,value",token,value)
      self[token] = value
    end
  end
...
function xNoteColumn:set_panning_string(str)
  TRACE("xNoteColumn:set_panning_string(str)",str)
...
function xNoteColumn.note_string_to_value(str_val)
  print("xNoteColumn.note_string_to_value(str_val)",str_val)
...
function xNoteColumn.note_value_to_string(val)
  print("xNoteColumn.note_value_to_string(val)",val)
...
-- @return int (0-255)
--[[
function xNoteColumn.volume_string_to_value(str_val)
  TRACE("xNoteColumn.volume_string_to_value(str_val)",str_val)
...
function xNoteColumn.volume_value_to_string(val)
  TRACE("xNoteColumn.volume_string_to_value(str_val)",str_val)
...
function xNoteColumn.column_value_to_string(val,empty)
  ...
  if (val == xLinePattern.EMPTY_VALUE) then
    --print("got here",empty)
    return empty
  end

  local numeric = val <= 0x80
  if numeric then
    --print("got here 2",("%.2X"):format(val))
    return ("%.2X"):format(val)
  else
    --print("xNoteColumn.convert_fx_to_string(val)",xNoteColumn.convert_fx_to_string(val))
...
-------------------------------------------------------------------------------
-- Write methods (a function for each token)
-------------------------------------------------------------------------------
-- @param note_col (renoise.NoteColumn), 
...
function xNoteColumn:do_write_note_value(note_col,clear_undefined)
  ...
  --print("*** xNoteColumn:do_write_note_value - self.note_value",self.note_value)
