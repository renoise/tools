...
function App.guess_path_to_exe()
  ...
  elseif (platform == "LINUX") then
    local cmd = 'find /home -path "*/sononym-*" -prune -print -quit 2>/dev/null'
    local f = assert(io.popen(cmd, 'r'))
    local s = assert(f:read('*a'))
    f:close()
    if s:len() > 0 then
      return cString.trim(s).."/sononym"
    else
      error("not found")
    end
  end
...
function App.guess_path_to_config()
  ...
  elseif (platform == "LINUX") then
    local cmd = 'find /home -path "*/Sononym/query.json" -prune -print -quit 2>/dev/null'
    local f = assert(io.popen(cmd, 'r'))
    local s = assert(f:read('*a'))
    f:close()
    if s:len() > 0 then
      return cString.trim(s)
    else
      error("not found")
    end
  end
